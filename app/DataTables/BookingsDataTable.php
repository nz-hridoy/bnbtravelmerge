...
class BookingsDataTable extends DataTable
{
    public function ajax()
    {
        ...
        return datatables()
            ...
            ->addColumn('code', function ($bookings) {
    ...
    public function query()
    {
        ...
        $bookings = Bookings::join('properties', function ($join) {
                        ...
                        ->select(['bookings.id as id', 'u.first_name as host_name', 'users.first_name as guest_name', 'bookings.property_id as property_id', 'properties.name as property_name', 'bookings.code as code', DB::raw('CONCAT(currency.symbol, bookings.total) AS total_amount'), 'bookings.status', 'bookings.created_at as created_at', 'bookings.updated_at as updated_at', 'bookings.start_date', 'bookings.end_date', 'bookings.guest', 'bookings.host_id', 'bookings.user_id', 'bookings.total', 'bookings.currency_code', 'bookings.service_charge', 'bookings.host_fee', 'bookings.iva_tax', 'bookings.accomodation_tax']);
        if (isset($user_id)) {
    ...
    public function html()
    {
        return $this->builder()
            ...
            ->addColumn(['data' => 'code', 'name' => 'bookings.code', 'title' => 'Confirmation Code'])
