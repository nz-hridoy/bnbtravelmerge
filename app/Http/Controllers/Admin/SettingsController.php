...
class SettingsController extends Controller
{
    ...
    public function fees(Request $request)
    {
        if ($request->isMethod('post')) {
            $rules = array(
                    ...
                    'host_fee'  => 'required|numeric|max:99|min:0',
            ...
            if ($validator->fails()) {
                ...
                PropertyFees::where(['field' => 'host_service_charge'])->update(['value' => $request->host_fee]);
    ...
    public function deleteBackground(Request $request)
    {
        ...
        if (isset($background)) {
            $record = Settings::where('name', 'static_bg')->first();
            if ($record) {
                $record->value=null;
                $record->save();
                ...
                $data['success']  = 1;
                $data['message'] = 'Background image has been deleted successfully!';
            } else {
                $data['success']  = 0;
                $data['message'] = "No Record Found!";
            }
