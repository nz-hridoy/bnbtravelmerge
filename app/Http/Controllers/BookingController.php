...
class BookingController extends Controller
{
    ...
    */
    ...
    public function dispute(Request $request, EmailController $email)
    {

        $booking                  = Bookings::find($request->bookingg_id);
        $booking->disputed_at     = date('Y-m-d H:i:s');
        $booking->save();

        // $booking_details             = new BookingDetails;
        // $booking_details->booking_id = $request->id;
        // $booking_details->field      = 'dispute_reason';
        // $booking_details->value      = ($request->dispute_reason == 'other') ? $request->dispute_reason_other : $request->dispute_reason;
        // $booking_details->save();

        $dispute = new Dispute();
        $dispute->booking_id = $booking->id;
        $dispute->property_id = $booking->property_id;
        $dispute->guest_id = $booking->user_id;
        $dispute->host_id = Auth::user()->id;
        $dispute->confirmation_code = $booking->code;
        $dispute->comment = $request->dispute_message;
        $dispute->save();

        // $status = 'Dispute';
        // $email->bookingAcceptedOrDeclined($request->bookingg_id, $status);
        // $companyName = Settings::where(['type' => 'general', 'name' => 'name'])->first(['value'])->value;
        // $requestBookingDecline = ($companyName.': ' .'Your booking for'.' '.$booking->properties->name .' '.'is Disputed.');
        // twilioSendSms($booking->users->formatted_phone, $requestBookingDecline);
        
        $this->helper->one_time_message('success', trans('Disputed successfully'));
        return redirect()->back();
    }

    public function cancelBooking(Request $request, EmailController $email)
    {

        $booking                  = Bookings::find($request->booking_cancel_id);
        ...
        $this->helper->one_time_message('success', trans('Cancelled successfully'));
        return redirect()->back();
    }
