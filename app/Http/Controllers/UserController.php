...
class UserController extends Controller
{
    ...
    public function dashboard()
    {
        ...
        $data['trip']           = Bookings::where(['user_id' => $user_id, 'status' => 'Paid'])->count();

        $bookings               = Bookings::select('payment_method_id', 'currency_code', 
                                DB::raw('(total - service_charge - iva_tax - accomodation_tax) as total'), 'created_at')->where(['host_id' => $user_id, 'status' => 'Paid']);
        ...
        $data['bookings']        = Bookings::with('users', 'properties')->where(['host_id' => $user_id, 'status' => 'Paid'])->orderBy('id', 'desc')->take(5)->get();
