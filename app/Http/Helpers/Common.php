...
class Common {
    ...
    public function get_price($property_id, $checkin, $checkout, $guest_count)
    {
       ...
       // $result['total_night_price'] = $remaining_day_price+$month_price-round(($month_price*$price_details->monthly_discount)/100)+$week_price-round(($week_price*$price_details->weekly_discount)/100);
        ...
        $result['total']            = $result['service_fee'] + $result['total_night_price'] + $result['additional_guest'] + $result['cleaning_fee'] + $result['iva_tax'] + $result['accomodation_tax'] - $result['discount'];
        $result['subtotal']         = $result['service_fee'] + $result['total_night_price'] + $result['additional_guest'] + $result['cleaning_fee'] +$result['iva_tax'] + $result['accomodation_tax'] - $result['discount'];
