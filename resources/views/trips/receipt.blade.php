        ...
        <thead class="thead-dark">
          <tr>
            @if ($booking->host_id == Auth::user()->id)
                <th colspan="6">{{trans('Payout Amount')}}</th>
            @else
                <th colspan="6">{{trans('Receipt')}}</th>
            @endif
          </tr>
        </thead>
        <tbody class="border">
          ...
          @if ($booking->discount != '')
              <tr>
                ...
                <td class="text-right pr-4">{!! $booking->currency->symbol !!} {{ $booking->discount }}</td>
              </tr>
          @endif
          ...
          @php
            $total = $booking->total-$booking->guest_charge;
            $host_total = $booking->total-$booking->service_charge-$booking->guest_charge;
          @endphp
          ...
          @if ($booking->host_id == Auth::user()->id)
            <tr>
              ...
              <td class="text-right pr-4">{!! $booking->currency->symbol.$host_total !!}</td>
            </tr>

            <tr>
              <td>Host Service Fee</td>
              <td class="text-right pr-4">-{!! $booking->currency->symbol.$booking->host_fee !!}</td>
            </tr>
          @else
            <tr>
              <td>{{trans('messages.trips_receipt.total')}}</td>
              <td class="text-right pr-4">{!! $booking->currency->symbol.$total !!}</td>
            </tr>
          @endif
        ...
        <div class="col-lg-3 col-sm-5 ml-auto pr-0">
          <table class="table table-clear">
            <tbody>
              <tr>
                <td class="left">
                  <strong>{{trans('Total Payout')}}:{{ $booking->receipt_date }}</strong>
                </td>
                @php
                  $total_payout = $host_total-$booking->host_fee;
                @endphp
                <td class="text-right pr-4"> {!! $booking->transaction_id ?  $booking->currency->symbol.$total_payout: 0 !!}</td>
              </tr>
