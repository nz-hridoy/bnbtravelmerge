...
<style>
	...
	@media (max-height: 680px) and (min-height: 510px) {
		#mainDiv{
			margin-bottom: 50px;
		}
	}

	@media (max-height: 509px) and (min-height: 400px) {
		#mainDiv{
			margin-bottom: 100px;
		}
	}

	@media (max-height: 399px) and (min-height: 270px){
		#mainDiv{
			margin-bottom: 150px;
		}
	}

	@media (max-height: 269px) and (min-height: 170px){
		#mainDiv{
			margin-bottom: 175px;
		}
	}
...
<div class="container-fluid container-fluid-90">
				...
				<div class="d-flex border rounded-4 p-4 mt-4">
					<div class="text-center">
						<a href="{{ url('users/show/'.$result->host_id) }}" >
							<img alt="User Profile Image" class="img-fluid rounded-circle mr-4 img-90x90" src="{{ $result->users->profile_src }}" title="{{$result->users->first_name}}" onerror="this.onerror=null;this.src='{{ asset('public/images/placeholderuser.jpg') }}';">
						</a>
					</div>
							...
							<div class="row">
								<div class="col-md-12  p-4 single-border border-r-10 ">
									<div class="row p-2" id="daterange-btn">
										...
										<input type="hidden" id="room_blocked_dates" value="11/24/2021" >
						...
						<div id="book_it" class="mt-4">
							...
							<ul class="list-inline text-center d-flex align-items-center justify-content-center">
								<li class="list-inline-item">
									...
									@php
										echo '<iframe src="https://www.facebook.com/plugins/share_button.php?href='.$shareLink.'&title='.$title.'&layout=button&size=large&mobile_iframe=true&width=73&height=28&appId" width="76" height="28" class="overflow-hidden border-0" scrolling="no" frameborder="0" allowTransparency="true" allow="encrypted-media"></iframe>';
									@endphp
								</li>
	...
	<div class="row mt-4 mt-sm-0">
		<div class="col-lg-8 col-xl-9 col-sm-12">
			<div class="row justify-content-center border rounded pb-5" id="listMargin"> {{-- Had a id 'ListMargin' --}}
									...
									<div class="col-md-6 col-sm-6">
										...
										<div>
											...
											@if($result->property_price->weekly_discount != 0)
												<strong> <span id="weekly_price_string">{!! moneyFormat('', $result->property_price->weekly_discount) !!}</span> /{{trans('messages.property_single.week')}}</strong>
											@else
												<strong><span id="weekly_price_string">{!! moneyFormat('', $result->property_price->weekly_discount) !!}</span> /{{trans('messages.property_single.week')}}</strong>
											@endif
										</div>
									...
									<div class="col-md-6 col-sm-6">
										<div>
											...
											@if($result->property_price->monthly_discount != 0)
												<strong> 
													<span id="weekly_price_string">{!! moneyFormat('', $result->property_price->monthly_discount) !!}</span> /{{trans('messages.property_single.month')}}
												</strong>
											@else
												<strong><span id="weekly_price_string">{!! moneyFormat('', $result->property_price->monthly_discount) !!}</span> /{{trans('messages.property_single.month')}}</strong>
											@endif
										</div>
