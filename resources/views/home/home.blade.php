...
@section('main')
				...
				<div class="row align-items-center text-center text-md-left">
					<div class="col-md-6 col-lg-5 mb-5 mb-md-0">
						<div class="main_formbg item animated zoomIn mt-80">
							<h1 class="pt-4 ">{{ trans($setting->s1_title) }}</h1>
		...
		<div class="container-fluid container-fluid-90">
			<div class="row">
				<div class="section-intro text-center">
					<p class="item animated fadeIn text-24 font-weight-700 m-0 text-capitalize">{{trans($setting->s2_title)}}</p>
					<p class="mt-3">{{trans($setting->s2_text)}} </p>
				</div>
			</div>
			...
			<div class="container-fluid container-fluid-90">
				<div class="row">
					<div class="recommandedhead section-intro text-center mt-70">
						<p class="item animated fadeIn text-24 font-weight-700 m-0">{{trans($setting->s3_title)}}</p>
						<p class="mt-2">{{trans($setting->s3_text)}}</p>
					</div>
				</div>
								...
								<div class="d-flex">
									<div>
										<div class="profile-img pl-2">
											<a href="{{ url('users/show/'.$property->host_id) }}"><img src="{{ $property->users->profile_src }}" alt="{{ $property->name}}" class="img-fluid" onerror="this.onerror=null;this.src='{{ asset('public/images/placeholderuser.jpg') }}';"></a>
										</div>
									</div>
	...
	<section class="testimonialbg pb-70" style="background-image: url(public/front/images/logos/{{ App\Models\Settings::where('name', 'static_bg')->first()->value }})">
		<div class="testimonials">
			<div class="container">
				<div class="row">
					<div class="section-intro text-center">
						<p class="item animated fadeIn text-24 font-weight-700 m-0 text-capitalize pb-3" style="padding-top: 100px; background: transparent; box-shadow: none;">{{trans($setting->s4_title)}}</p>
						<p class="mt-3">{{trans($setting->s4_text)}} </p>
					</div>
				</div>
					...
					<div class="col-md-12 mt-5 mb-5 text-center">
						<a class="nav-link p-2" href="{{ url('property/create') }}" aria-label="property-create">
							<button class="btn vbtn-outline-success text-14 font-weight-700 p-0 mt-2 pl-4 pr-4">
								<p class="p-3 mb-0">  {{trans('List Your Property')}}</p>
							</button>
						</a>
					</div>   
				</div>
				...
				<div class="row">
					<div class="section-intro text-center">
						<p class="item animated fadeIn text-24 font-weight-700 m-0 text-capitalize pb-3" style="background: transparent; box-shadow: none;">{{trans($setting->s5_title)}}</p>
						<p class="mt-3">{{trans($setting->s5_text)}} </p>
						<div class="col-md-12">
							<div class="social mt-4">
								<ul class="list-inline text-center">
									...
									<a class="social-icon text-color text-30" target="_blank" href="{{ $join_us[$i]->value }}" aria-label="{{$join_us[$i]->name}}"><i class="fab fa-{{ str_replace('_','-',$join_us[$i]->name) }}"></i></a>
									</li>
									@endfor
									@endif  
								</ul>
	...
	<section class="bg-gray mt-5 mb-5">
		<div class="container-fluid container-fluid-90">
			<div class="row">
				<div class="recommandedhead section-intro text-center mt-70">
					<p class="animated fadeIn text-24 text-color font-weight-700 m-0">{{ trans($setting->s6_title) }}</p>
					<p class="mt-2">{{trans($setting->s6_text)}}</p>
				</div>
			</div>
