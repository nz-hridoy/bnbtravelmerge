...
if($result['static_bg']==null)
{
    $static_background = "public/front/images/default-error-image.png";
}
else {
    $static_background = 'public/front/images/logos/'.$result['static_bg'];
}
$form_data = [
		...
		'fields' => [ 
            ...
            ['type' => 'file', 'class' => 'validate_field', 'label' => 'Homepage Static Background', 'name' => "photos[static_bg]", 'value' => '', 'image' => url($static_background), 'custom_span3' =>$result['static_bg'], 'custom_company_background' => $result['static_bg']],
   ...
   $(document).ready(function () {
            ...
            $('#general_form').validate({
                ...
                messages: {
                    ...
                    'photos[static_bg]': {
                        extension: 'The file must be an image (jpg, jpeg, png)'
                    }
                } 
            });
        ...
        $('.remove_background_preview').on("click", function(){
        var image = $('#hidden_company_background').attr('data-rel');
        var token = $('input[name="_token"]').val();
        if(image){
                $.ajax({
                url : "settings/delete_background",
                type : "post",
                async : false,
                data: { 'company_background' : image, '_token' : token},
                dataType : 'json',
                success: function(reply)
                {
                    if (reply.success == 1){
